    ...
    <div style={{
      ...
      {items.map(item => (
        <div key={item.id} onClick={item.action} style={{
          ...
          {active === item.id && (
            <div style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)', width: '24px', height: '2px', background: '#D95C1A', borderRadius: '0 0 2px 2px' }} />
          )}
          <div style={{ fontSize: '22px', color: active === item.id ? '#D95C1A' : '#444' }}>{item.icon}</div>
          <div style={{
            fontSize: '10px', letterSpacing: '0.8px',
            fontFamily: 'Barlow Condensed, sans-serif', fontWeight: '700',
            color: active === item.id ? '#D95C1A' : '#444',
            textAlign: 'center',
          }}>{item.label}</div>
        </div>
      ))}
